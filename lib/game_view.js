...
class GameView {
  constructor(renderer, camera, scene, key, musicDelay) {
    ...
    this.video = new Video(musicDelay);
    this.paused = false;
    ...
    this.video.videoEl.addEventListener('waiting', function () {
      this.pausePlay();
      this.paused = true;
    });

    this.video.videoEl.addEventListener('playing', function () {
      if (this.paused) this.pausePlay();
    });
  }
  ...
  backgroundSetup() {
    ...
    let backgroundGeometry = new THREE.PlaneGeometry(180, 90);
    let texture = new THREE.VideoTexture(this.video.videoEl)
